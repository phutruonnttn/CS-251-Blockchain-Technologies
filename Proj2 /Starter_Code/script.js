...
var contractAddress = '0xe4e47451AAd6C89a6D9E4aD104A7b77FfE1D3b36'; // FIXME: fill this in with your contract's address/hash
...
async function getUsers() {
    // a list of everyone who has ever sent or received an IOU
    var list = await BlockchainSplitwise.methods.getListUser().call();
    return list;
}

// TODO: Get the total amount owed by the user specified by 'user'
async function getTotalOwed(user) {
    var list = await getListCreditors(user);
    var total = 0;
    for (let i = 0; i < list.length; i++) {
        let v = list[i];
        total += Number(await BlockchainSplitwise.methods.lookup(user, v).call());
    }
    return total;
}
...
async function getLastActive(user) {
    var listFunc = await getAllFunctionCalls(contractAddress, "add_IOU");
    for (let i = 0; i < listFunc.length; i++) {
        if ((listFunc[i].from == user.toLowerCase()) || (listFunc[i].args[0] == user.toLowerCase())) {
            return listFunc[i].t;
        }
    }
    return null;
...
async function add_IOU(creditor, amount) {
    ...
    var oke = true;
    for (let i = 0; i < list.length; i++) {
        if (list[i].toLowerCase() == creditor.toLowerCase()) {
            oke = false;
            break;
        }
    }
    if (oke) {
        await BlockchainSplitwise.methods.addToListUser(creditor).send({ from: web3.eth.defaultAccount });
    }

    list = await BlockchainSplitwise.methods.getListUser().call();
    oke = true;
    for (let i = 0; i < list.length; i++) {
        if (list[i].toLowerCase() == web3.eth.defaultAccount.toLowerCase()) {
            oke = false;
            break;
        }
    }
    if (oke) {
        await BlockchainSplitwise.methods.addToListUser(web3.eth.defaultAccount).send({ from: web3.eth.defaultAccount });
    }

    var flag = await check_cycle(web3.eth.defaultAccount);
    var acc = web3.eth.defaultAccount;
    if (flag) {
        let min = amount;
        let next = acc.toLowerCase();
        let debt;
        do {
            debt = await BlockchainSplitwise.methods.lookup(trace.get(next), next).call();
            if (Number(min) > Number(debt)) min = debt;
            next = trace.get(next);
        } while (next != acc.toLowerCase());

        next = acc.toLowerCase();
        do {
            debt = await BlockchainSplitwise.methods.lookup(trace.get(next), next).call();
            await BlockchainSplitwise.methods.updateStored(trace.get(next), next, debt - min).send({ from: web3.eth.defaultAccount });
            next = trace.get(next);
        } while (next != acc.toLowerCase());
    }
} //dang comment phan reload trang lai de debug

async function getListCreditors(addr) {
    return BlockchainSplitwise.methods.getListCreditors(addr).call();
}

var cycle, visit, trace;

async function dfs(u) {
    visit.set(u, 1); // đã được thăm trong quá trình duyệt cây dfs gốc u
    var list = await getListCreditors(u);
    for (let i = 0; i < list.length; i++) {
        let v = list[i];
        trace.set(v.toLowerCase(), u.toLowerCase());
        if (!visit.has(v)) {
            await dfs(v);
        } else if (visit.get(v) == 1) {
            cycle = true;
        }
    }
    visit.set(u, 2);
