...
function computeInput(depth, transcript, nullifier) {
    ...
    for (var i = 0; i < transcript.length; i++) {
        if (transcript[i].length == 1) {
            smt.insert(transcript[i][0]);
        } else {
            smt.insert(mimc2(transcript[i][0], transcript[i][1]));
            if (transcript[i][0] == nullifier) {
                nonce = transcript[i][1];
                leaf = mimc2(transcript[i][0], transcript[i][1]);
            }
        }
    }
    ret.digest = smt.digest;
    ret.nullifier = nullifier;
    ret.nonce = nonce;

    if (smt.path(leaf).length > 0) {
        smt.path(leaf).map((item, index) => { ret[`sibling[${index}]`] = item[0], ret[`direction[${index}]`] = item[1] == true ? "1" : "0" });
    }
    return ret;
}
